...
public class Alphametics {
    ...
    Map<Character, Integer> solve() throws UnsolvablePuzzleException {
        List<Character> letters = puzzle.chars()
                                        .filter(Character::isAlphabetic)
                                        .mapToObj(ch -> (char) ch)
                                        .distinct()
                                        .collect(Collectors.toList());
        int[] digits = IntStream.range(0, 10)
                                .toArray();
    ...
    Map<Character, Integer> search(List<Character> letters, int[] digits, int index) {
        if (index == letters.size()) {
            Map<Character, Integer> letter2digit = IntStream.range(0, letters.size())
                                                            .boxed()
    ...
    boolean check(Map<Character, Integer> letter2digit) {
        ...
        for (Map.Entry<Character, Integer> entry : letter2digit.entrySet()) {
            translated = translated.replace(entry.getKey(), (char) (entry.getValue() + '0'));
        }

        String[] items = translated.split("\\W+");

        if (Arrays.stream(items)
                  .anyMatch(item -> item.length() > 1 && item.startsWith("0"))) {
            return false;
        }

        return Arrays.stream(items, 0, items.length - 1)
                     .mapToLong(Long::parseLong)
                     .sum() == Long.parseLong(items[items.length - 1]);
    }
}
