...
public class Tournament {
    Map<String, Team> name2team = new HashMap<>();
    ...
    String printTable() {
        ...
        List<Team> teams = new ArrayList<>(name2team.values());
        Collections.sort(teams, (team1, team2) -> (team1.getP() != team2.getP()) ?
                Integer.compare(team2.getP(), team1.getP()) :
                team1.name.compareTo(team2.name));
        for (Team team : teams) {
            result.append(
                    String.format("%-30s | %2d | %2d | %2d | %2d | %2d", team.name, team.getMP(), team.w, team.d, team.l, team.getP()));
            result.append("\n");
        }
